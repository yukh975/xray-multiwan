...
Один Linux-хост с несколькими связками xray+tun2socks — по одной на WAN-выход. Клиент выбирает выход, указывая разные IP в качестве шлюза: например, `.101` → первый выход, `.102` → второй, `.103` → третий.

Хост может быть любым — LXC-контейнер Proxmox, виртуалка на любом гипервизоре, отдельный физический сервер. Схема разрабатывалась на Proxmox-контейнере (поэтому в «Предпосылках» ниже упоминается LXC и проброс TUN), но сама по себе она опирается только на сетевые примитивы Linux (macvlan, iptables, policy routing, tun). Где именно это запускать — не важно; важен принцип и схема работы.

## Содержание

- [Архитектура](#архитектура)
- [Предварительные требования](#предварительные-требования)
- [Перед первым запуском: правим `config.sh`](#перед-первым-запуском-правим-configsh)
- [Удаление](#удаление)
- [Пошаговая установка](#пошаговая-установка)
- [Диагностика](#диагностика)
- [Мониторинг туннелей (tunnel-monitor)](#мониторинг-туннелей-tunnel-monitor)
  - [Установка](#установка)
  - [Конфигурация](#конфигурация-tunnel-monitorconf)
  - [Проверка](#проверка)
- [Повторный запуск install.sh поверх рабочей системы](#повторный-запуск-installsh-поверх-рабочей-системы)
- [Добавление нового выхода](#добавление-нового-выхода)
- [Миграция на новую адресацию](#миграция-на-новую-адресацию)
- [Автор](#автор)
...
Клиент (192.168.0.100)
  │
  │ gateway = 192.168.0.101 (или .102, .103)
  ↓
┌────────────────────────────────────────────────────────┐
...
2. Клиент, указав шлюз `.101`, через ARP получает MAC `xray-wan1` — и кадры летят именно на этот интерфейс.
...
3. **xray** установлен и оформлен как systemd-шаблон `xray@.service`, по одному инстансу на выход:
   - `xray@wan1` слушает socks5 на `192.168.0.101:10808`
   - `xray@wan2` слушает socks5 на `192.168.0.102:10808`
   - `xray@wan3` слушает socks5 на `192.168.0.103:10808`
...
EXITS=(
    "wan1:192.168.0.101:100"              # код:IP:fwmark
...
1. Напиши xray-конфиг в `/etc/xray/<код>.json` с outbound (VLESS/VMess/Trojan/и т.д.) на upstream и SOCKS inbound, слушающим на macvlan-IP этого выхода (например `192.168.0.101:10808`). Разные выходы **обязаны** слушать на разных macvlan-IP — иначе поднимется только один инстанс.
...
Открой `https://api.ipify.org` в браузере — должен показать IP первого upstream'а. Меняй шлюз на `.102` / `.103` — получишь IP второго и третьего.
...
Если нужно поменять список выходов — отредактируй массив `EXITS` в [`config.sh`](config.sh) и запусти `install.sh` заново. Скрипт перенастроит всё, включая удаление правил для выходов, которых больше нет, только для тех артефактов, которые он контролирует (скрипты, юниты, mangle-правила — они полностью перегенерируются).
...
Либо проще: добавить новый выход в массив `EXITS` в [`config.sh`](config.sh) и запустить `install.sh` заново — он идемпотентен и перенастроит всё с учётом нового выхода.
...
В самом `migrate.sh` править нечего — он читает целевой план из [`config.sh`](config.sh). Отредактируй `config.sh` под новое состояние (новый `NETMASK_BITS` и новые IP для каждого WAN-выхода — каждый выход это произвольный xray outbound, не обязательно привязанный к стране) и запусти `migrate.sh`. Коды выходов и fwmark'и не меняются — только IP и префикс.
...
1. **Проверки** — `eth0` в подсети первого элемента `EXITS` (грубое сравнение по /24); `install.sh`, `diag.sh`, `config.sh` есть по ожидаемым путям (`/root/files/` по умолчанию, переопределяется через `INSTALL_SH=` / `DIAG_SH=` / `CONFIG_SH=`).
...
- **Не** удаляет macvlan-интерфейсы тех выходов, которых больше нет в `EXITS` — снимаются только IP.
