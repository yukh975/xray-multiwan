...
Если нужно поменять список выходов — отредактируй массив `COUNTRIES` в начале скрипта и запусти заново. Скрипт перенастроит всё, включая удаление правил для выходов, которых больше нет, только для тех артефактов, которые он контролирует (скрипты, юниты, mangle-правила — они полностью перегенерируются). Имя массива осталось `COUNTRIES` исторически; концептуально это WAN-выходы.
...
Либо проще: отредактировать массив `COUNTRIES` в `install.sh` и запустить его заново — он идемпотентен и перенастроит всё с учётом нового выхода.
