...
Exit code: `0` — всё зелёное, `1` — есть падения. Массив `COUNTRIES` в начале `diag.sh` должен совпадать с `install.sh` — `migrate.sh` синхронизирует их автоматически.
...
- **ip rule** — правило fwmark → таблица присутствует для каждой страны.
...
- **Реальная связность** — `curl --interface tun<код> https://api.ipify.org` для каждой страны + проверка, что все exit-IP разные.
...
`migrate.sh` — для случая, когда меняется вся подсеть: провайдер переделал IP-план, или контейнер переезжает на другой бридж с другим диапазоном. Скрипт за один проход переписывает xray-конфиги, tun2socks-конфиги, `install.sh`, `diag.sh` и живые macvlan-интерфейсы.
...
- `NEW_COUNTRIES` — тот же формат `код:IP:mark`, что и в `install.sh`. Коды стран и fwmark'и не меняются, меняются только IP.
...
1. **Проверки** — `global` в `NEW_PARENT_IP/NEW_NETMASK_BITS`; `install.sh`, `diag.sh` есть по ожидаемым путям (`/root/files/` по умолчанию, переопределяется через `INSTALL_SH=` / `DIAG_SH=`).
2. **Бэкап** — `/etc/xray/<код>.json`, `/etc/tun2socks/<код>.yaml`, `install.sh`, `diag.sh` копируются в `/root/migrate-backup-<YYYYMMDD-HHMMSS>/`.
3. **xray-конфиги** — в каждом `/etc/xray/<код>.json` переписывается поле `"listen"` на новый IP страны.
4. **tun2socks-конфиги** — в каждом `/etc/tun2socks/<код>.yaml` переписывается `proxy: socks5://<старый-ip>:10808` на новый.
5. **install.sh / diag.sh** — через `awk` переписываются массив `COUNTRIES` и значение `NETMASK_BITS` (старый блок удаляется, новый вставляется на его место).
...
- **Не** удаляет macvlan-интерфейсы тех стран, которых больше нет в `NEW_COUNTRIES` — снимаются только IP.
